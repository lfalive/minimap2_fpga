...
void ksw_extd2_sse2(void *km, int qlen, const uint8_t *query, int tlen, const uint8_t *target, int8_t m, const int8_t *mat,
				   int8_t q, int8_t e, int8_t q2, int8_t e2, int w, int zdrop, int end_bonus, int flag, ksw_extz_t *ez)
#endif
#else//ksw_extd2_sse(km, qlen, qseq, tlen, tseq, 5, mat, opt->q, opt->e, opt->q2, opt->e2, w, zdrop, end_bonus, flag, ez);
void ksw_extd2_sse(void *km, int qlen, char *query, int tlen, char *target, int8_t m, const int8_t *mat,
                   int8_t q, int8_t e, int8_t q2, int8_t e2, int w, int zdrop, int end_bonus, int flag, ksw_extz_t *ez)
...
#define __dp_code_block2 \
    ...
    if (q2 + e2 < q + e) t = q, q = q2, q2 = t, t = e, e = e2, e2 = t; // make sure q+e no larger than q2+e2 q=12 q2=24 e=2 e2=1
    ...
    sc_mch_ = _mm_set1_epi8(mat[0]);//match=2
    sc_mis_ = _mm_set1_epi8(mat[1]);//mismatch =
    ...
    long_thres = e != e2? (q2 - q) / (e - e2) - 1 : 0;//long_thres=11
    if (q2 + e2 + long_thres * e2 > q + e + long_thres * e)
        ++long_thres;
    long_diff = long_thres * (e - e2) - (q2 - q) - e2;//cj:long_diff=-2
    ...
    for (t = 0; t < qlen; ++t) qr[t] = query[qlen - 1 - t];//cj:qr query reverse
    ...
    for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
        ...
        int cj=0;
        //for (cj=0; cj<r; ++cj) printf("%d ", H[cj]);
