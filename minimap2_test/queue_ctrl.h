...
#define US_PSIZE_EXP	12	// package size 4KiB
...
void *printDebugInfo(void* args){
    while(1){
        usleep(1000);
        FILE *file;
        file = fopen("../../Output/test.log","a+");
        if (!file) perror("../../Output/test.log"), exit(1);
        fprintf(file, "---------------------------------\n");
...
void WriteDsPkg(void *buf, uint pkgCnt)
{
    ...
    if (rc < 0) {
        cerr<<"err when write DDR at address at "<<dsTail<<endl;
    }
    else{
        //printf(" writePkg at DDR address : %d",dsTail);
    }
    updateDsTail(pkgCnt);
}
...
void ReadUsPkg(void *buf, uint pkgCnt)
{
    ...
    if(rc < 0){
        cerr<<"err when read DDR at address at "<<usHead<<endl;
    }/*else
        printf("readPkg at DDR address : %p\n",usHead);*/
    updateUsHead(pkgCnt);
}
int OpenQueueCtrl()
{
    regFd = open("/dev/xdma0_user", O_RDWR | O_SYNC);
    regBase = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, regFd, 0);//addr =0 len = 32K
