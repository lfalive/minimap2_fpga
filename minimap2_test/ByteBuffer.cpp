...
// Read Functions
    ...
    uint8_t ByteBuffer::getLittle() const {
        uint8_t temp = read<uint8_t>();
        return temp;
    }
    ...
    uint32_t ByteBuffer::getIntLittle() const {
        int temp=0;
        for (uint32_t i = 0; i < 4; i++) {
            temp = (temp << 8) | read<uint8_t>();
        }
        return temp;
    }
    ...
    uint64_t ByteBuffer::getLongLittle() const {
        long temp=0;
    ...
    uint16_t ByteBuffer::getShortLittle() const {
        uint16_t temp=0;
...
// Write Functions
    ...
    void ByteBuffer::putInt_Little(uint32_t value) {
        for (int i = 0; i < 4; i++) {
            char temp = value >> (8 * (3 - i));
            append<char>(temp);
        }
    }
    ...
    void ByteBuffer::putLong_Little(uint64_t value) {
        for (int i = 0; i < 8; i++) {
            char temp = value >> (8 * (7 - i));
            append<char>(temp);
        }
    }
    ...
    void ByteBuffer::putShort_Little(uint16_t value) {
        for (int i = 0; i < 2; i++) {
            char temp = value >> (8 * (1 - i));
            append<char>(temp);
        }
    }
