...
// gpu-server-5
 #define NSTREAM 128 // 1k
// #define NSTREAM 128 // 2k
// #define NSTREAM 128 // 4k
// #define NSTREAM 84 // 8k
// #define NSTREAM 48 // 10k
// #define NSTREAM 20 // 16k
// #define NSTREAM 12 // 20k
// #define NSTREAM 4 // 32k

 #define MAXMEM 2200000 // 1k 2016000
// #define MAXMEM 8400000 // 2k, 8032000
// #define MAXMEM 32400000 // 4k, 32064000
// #define MAXMEM 129000000UL // 8k, 128128000+64000
// #define MAXMEM 202200000UL // 10k
// #define MAXMEM 560000000UL // 16k
// #define MAXMEM 820000000UL // 20k
// #define MAXMEM 2080000000UL // 32k

// gpu-server-2
// #define NSTREAM 128 // 1k
// #define NSTREAM 128 // 2k
// #define NSTREAM 128 // 4k
// #define NSTREAM 128 // 8k
// #define NSTREAM 112 // 10k
// #define NSTREAM 36 // 16k
// #define NSTREAM 28 // 20k
// #define NSTREAM 10 // 32k
...
__global__ void ksw_extd2_kernel(void *gm, int qlen, const uint8_t *query, int tlen, const uint8_t *target,
	...
	int *off = 0, *off_end = 0;
	int8_t *u, *v, *x, *y, *x2, *y2, *p = 0;
	uint8_t *qr;
	const uint8_t *sf;

	n_col_ = qlen < tlen? qlen : tlen;
	n_col_ = ((n_col_ < w + 1? n_col_ : w + 1) + WIDTH - 1) / WIDTH + 1;
	...
	// off = (int*)(qr + qlen_ * WIDTH), off_end = off + qlen + tlen - 1;
	sf = target;

	// off = (int*)(((size_t)gm + WIDTH - 1) >> SHIFT << SHIFT);
	off = (int*)(qr + qlen_ * WIDTH);
	off_end = off + qlen + tlen - 1;
	p = (int8_t*)(off_end + qlen + tlen - 1);
	// p = (int8_t*)gm;
	...
	for (r = 0; r < qlen + tlen - 1; ++r) {
		...
		int8_t *pr = p + (size_t)r * n_col_ * WIDTH - st;
		for (t = st + tid; t <= en; t += elementskip) {
			int8_t d, z, a, b, a2, b2, vt, ut;
		...
		if (r == qlen + tlen - 2 && en == tlen - 1 && tid == 0)
			ez->score = H0;
	}
	if (!ez->zdropped && tid == 0) {
		ksw_backtrack_gpu(NULL, 1, 0, 0, (uint8_t*)p, off, off_end, n_col_*WIDTH, tlen-1, qlen-1, &ez->m_cigar, &ez->n_cigar, &ez->cigar);
	}
}
...
*/
	for (int i = 0; i < nkernel; ++i) {
		...
		ksw_extd2_kernel<<<nblock, nthread, 0, streams[sid]>>>((void*)((int8_t*)d_gm + sid * (size_t)MAXMEM), qlen, &d_query[sid * MAXLEN], tlen, &d_target[sid * MAXLEN], q, e, q2, e2, w, zdrop, &d_ez[i]);
